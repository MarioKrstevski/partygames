...
export default function WordSpyGame() {
  const { roster, update, ready } = usePlayers();

  const [phase, setPhase] = useState<Phase>("setup");
  const [categoryChoice, setCategoryChoice] = useState<string>(SURPRISE);
  ...
  const startRound = useCallback(() => {
    ...
    const category =
      categoryChoice === SURPRISE
        ? CATEGORIES[randomNumber(0, CATEGORIES.length - 1)]
        : CATEGORIES.find((c) => c.name === categoryChoice) ?? CATEGORIES[0];
    const word = category.words[randomNumber(0, category.words.length - 1)];
          ...
          <>
            <Card className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button
                  type="button"
                  variant={categoryChoice === SURPRISE ? "primary" : "secondary"}
                  className="col-span-2"
                  onClick={() => setCategoryChoice(SURPRISE)}
                >
                  🎲 Surprise me
                </Button>
                {CATEGORIES.map((c) => (
                  <Button
                    key={c.name}
                    type="button"
                    variant={categoryChoice === c.name ? "primary" : "secondary"}
                    onClick={() => setCategoryChoice(c.name)}
                  >
                    {c.emoji} {c.name}
                  </Button>
                ))}
      ...
      return (
        <PageContainer className="max-w-lg space-y-5">
          ...
          <Card className="space-y-4 py-10 text-center">
            ...
            <Button
              type="button"
              className="mx-auto w-full max-w-xs py-4 text-base"
              ...
              isSpy ? (
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-zinc-300">
                    Blend in! Category: {round.categoryName}
                  </p>
                </div>
              ) : (
                <div className="space-y-1">
      ...
      <PageContainer className="max-w-lg space-y-5">
        ...
        <Card className="space-y-2 text-sm text-zinc-300">
          <p>
            ...
            obvious and the spy learns the word; too cryptic and{" "}
            <em>you</em> look like the spy.
          </p>
